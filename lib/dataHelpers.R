...
dataHelper.getFirstDroneSnapshots = function (simulationID) {
  firstSS <- dataHelper.getFirstSimulationSnapshot(simulationID)
  return(
    data.droneSnapshots %>%
      filter(simulationSnapshot == firstSS$`_id`)
  )  
}
...
dataHelper.getLastDroneSnapshots = function (simulationID) {
  lastSS <- dataHelper.getLastSimulationSnapshot(simulationID)
  return(
    data.droneSnapshots %>%
      filter(simulationSnapshot == lastSS$`_id`)
  )  
}
...
dataHelper.isOutlier = function (simulationID) {
  ...
  return(
    (((dataHelper.getLastSimulationSnapshot(simulationID))$elapsedRealTime[[1]]) < Constants$OUTLIER_REAL_TIME_THRESHOLD) ||
...
##
dataHelper.getCurrentSelectedUAV <- function (ss) {
  #1. Get the previous snapshots (ordered in time) of the given ss
  previousInteractions <- dataHelper.getPreviousSimulationSnapshots(ss) %>%
    filter(cause.id == SnapshotCauses$USER_INPUT)
...
#
dataHelper.getAverageTimeBetweenInteractions <- function () {
