...
hsmmspec <- function(init,transition,parms.emission,sojourn,dens.emission,rand.emission=NULL,mstep=NULL) {
  ...
  if(all(sojourn$type!=c("nonparametric","gamma","poisson","lnorm","logarithmic","gaussian"))) stop(paste("Invalid sojourn type specified (",sojourn$type,")"))
...
#          }        
       fn <- function (ii) rlnorm(n=1, meanlog = object$sojourn$meanlog[ii], object$sojourn$s.dlog[ii])
     }    
    ...
    else if (object$sojourn$type == "gaussian") {
      fn <- function (ii) rnorm(n = 1, mean = object$sojourn$mean[ii], sd = object$sojourn$sd[ii])
    }
...
.build_d <- function(model,M) {
  ...
  if (sojourn.distribution == "gaussian") {
    if(is.null(model$sojourn$mean)) {
      if(is.null(model$sojourn$d))
        stop('Invalid waiting parameters supplied')
      else model$d = model$sojourn$d
    }
      ...
      model$d = matrix(nrow=M,ncol=model$J)
      for(i in 1:J) model$d[,i] = dnorm(1:M,mean = model$sojourn$mean[i],sd = model$sojourn$sd[i])
    }    
  }
...
hsmmfit <- function(x,model,mstep=NULL,M=NA,maxit=100,lock.transition=FALSE,lock.d=FALSE, tol=1e-4,graphical=FALSE,debug=FALSE) {
  
  ptm <- proc.time() #Time measure
  ...
  for(it in 1:maxit) {
    ...
    p = sapply(1:J,function(state) as.numeric(f(x,state,new.model))) #Added!!!
    ...
    if (debug) print(paste("Iteration",it))
    # E-STEP
    B  = .C("backward",
    ...
    new.model$parms.emission = mstep(x,state_wt,model=old.model)
    ...
    if(lock.d) {
      ...
      else if(sojourn.distribution == "gaussian") {
        eta = matrix(B$eta,ncol=J)
        new.model$d = matrix(nrow=M,ncol=J)
        new.model$sojourn$mean = numeric(J)
        new.model$sojourn$sd = numeric(J)
        for (i in 1:J) {
          tmp <- cov.wt(x = data.frame(1:M),wt = eta[,i])
          new.model$sojourn$mean[i] = tmp$center
          new.model$sojourn$sd[i] = tmp$cov
          new.model$d[,i] = dnorm(1:M,new.model$sojourn$mean[i],new.model$sojourn$sd[i])
          new.model$d[,i] = new.model$d[,i]/sum(new.model$d[,i])     
        }
      }
...
# Get the expected duration for a given state
expectedDuration.hsmm <- function(object, j) {
  if (object$model$sojourn$type == "gamma") {
    ans <- object$model$sojourn$scale[j]*object$model$sojourn$shape[j]
    
  } else if (object$model$sojourn$type == "poisson") {
    ans <- object$model$sojourn$lambda[j] + object$model$sojourn$shift[j]
    
  } else if (object$model$sojourn$type == "lnorm") {
    ans <- exp((object$model$sojourn$meanlog[j] + (object$model$sojourn$s.dlog[j])^2)/2)
  } else if (object$model$sojourn$type == "gaussian") {
    ans <- object$model$sojourn$mean[j]
  } else if (object$model$sojourn$type == "logarithmic") {
    ans <- (-1/log(1-object$model$sojourn$shape[j]))*(object$model$sojourn$shape[j]/(1-object$model$sojourn$shape[j]))
  } else {
    stop(paste("Expected duration cannot be computed for sojourn type : ",object$model$sojourn$type))
  }
  
  ans
}

# Get the expected duration deviation
durationDeviation.hsmm <- function(object,j) {
  if (object$model$sojourn$type == "gamma") {
    ans <- sqrt(object$model$sojourn$shape[j]*(object$model$sojourn$scale[j])^2)
    
  } else if (object$model$sojourn$type == "poisson") {
    ans <- sqrt(object$model$sojourn$lambda[j])
  } else if (object$model$sojourn$type == "lnorm") {
    #ans <- exp((object$model$sojourn$meanlog[j] + (object$model$sojourn$s.dlog[j])^2)/2)
    ans <- sqrt((exp((object$model$sojourn$s.dlog[j])^2)-1)*exp(2*object$model$sojourn$meanlog[j] + (object$model$sojourn$s.dlog[j])^2))
  } else if (object$model$sojourn$type == "gaussian") {
    ans <- object$model$sojourn$sd[j]
  } else if (object$model$sojourn$type == "logarithmic") {
    #ans <- (-1/log(1-object$model$sojourn$shape[j]))*(object$model$sojourn$shape[j]/(1-object$model$sojourn$shape[j]))
    stop(paste("Expected duration cannot be computed for sojourn type : ",object$model$sojourn$type))
  } else {
