...
missionHelper.getSimulationEnvironment = function (simulationID) {
  return(
    missionHelper.getEnvironmentById(
      (dataHelper.getFirstSimulationSnapshot(simulationID) %>%
         merge(y=data.simulations,by.x="simulation",by.y="simulationID")
       )$mission[[1]]
    )
  )
}
...
missionHelper.getMissionID = function (simulationID) {
  return(
    (data.simulations %>% filter(simulationID == simulationID))[1,]$mission
  )
}
