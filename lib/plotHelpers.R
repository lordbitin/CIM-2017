# Wrapper around igraph plot
#Input : HMM model from the library markovchain
plotHelper.plotMarkovChain <- function (initProbs,transitionMatrix,stateNames=NULL,edgeThreshold=0.05, name="New Markov Chain",...) 
{
#Plot constants

#Aux function to pass MC -> IGRAPH
...
#Create a MarkovChain object with the values given in the function parameters
if (is.null(stateNames)) {
  stateNames <- laply(1:length(initProbs), function (i) paste("state",i))
}
stateNames <- paste(stateNames,"\n(",round(initProbs,2),")")
...
netMc <- .getNet(mc,round=TRUE)
netMc <- delete.edges(netMc,E(netMc)[weight < edgeThreshold*100])
edgeLabel <- round(E(netMc)$weight/100,2)
#layout <- layout.reingold.tilford(netMc, circular=T)
#layout=layout.fruchterman.reingold(netMc, niter=10000, area=30*vcount(netMc)^2)
#layout=layout.fruchterman.reingold(netMc, niter=20000)
#layout <- layout.reingold.tilford(netMc)
...
##
##
plotHelper.plotBIC = function (data) {
  
  par(oma=c(3,3,0,0))
  par(cex.main=1.5, cex.axis=1.1, cex.lab=1.5)
  
  #Plot loglik
  plot(x=data$nstates,y=data$logLik,ylim=c(-2050,-1750),col="black",type="l",main="",xlab="nstates",ylab="", yaxt='n', lwd=2)
  axis(side=2,col="black",line=3)
  mtext("LogLik",side=2,line=3,col="black",cex=1.3,adj=-0.3)
  
  par(new=T)
  
  #Plot BIC
  plot(x=data$nstates,y=data$BIC,ylim=c(4000,4300),col="blue",type="l",lwd=2,xaxt='n',axes=F,ylab='',xlab='',lty="dashed")
  axis(side=2,col="blue",line=0)
  mtext("BIC",side=2,line=0,col="black",cex=1.3,adj=-0.25)
  
  lim = par("usr")
  
  #Mark the best state
  rect(4.7,lim[4]-10,5.3,lim[3]+10,border="red")
  
  legend(6.7,4075,c("logLik  ","BIC"),lty=c("solid","dashed"),lwd=2,col=c("black","blue"))
  
}

# Plot a Grid with the values of the emission functions for each state
# INPUT: List of vectors (Length of the list = number of states)
# Only valid for multinomial emission distributions
plotHelper.hmm.emissionDistributionGrid <- function (emissionParms,obsSymbols) {
  ...
  for (i in 1:nstates) {
    p <- p + layer(data= epDfLf %>% filter(Var2==paste("state",i)),  geom = c("line"), stat = "identity") + ylim(0,1)
  }
...
# Model selection
# 
plotHelper.hmm.modelSelection <- function (hmmList) {
  
  if (class(hmmList)!="list" || length(hmmList) == 0)
    stop("Bad parameters. Input [HmmList]")
  
  if (class(hmmList[[1]]) == "hmm") {
    #MHSMM Models
    nstateList <- laply(hmms,function (hmm) hmm$K)
  } else if (class(hmmList[[1]]) == "hsmm") {
    nstateList <- laply(hsmms,function (hsmm) hsmm$J)
  } else {
    #DepmixS$ HMMS?
    nstateList <- laply(hmmModels,nstates)
  }
  
  result <- data.frame(nstates=nstateList,
                       logLik=laply(hmmList,logLik),
                       BIC=laply(hmmList,BIC))
  
  #MAS??
  MASPlot <- FALSE
  if (!is.null(hmmList[[1]]$MAS)) {
    result$MAS <- laply(hmmList, function (hmm) hmm$MAS)
    MASPlot <- TRUE
  }
  
  resultLf <- melt(result,id.vars=c("nstates"))
  
  p <- ggplot(data = resultLf, mapping = aes(x = nstates, y = value))
  p <- p + scale_x_continuous(breaks=seq(min(nstateList),max(nstateList),1))
  p <- p + labs(y=NULL)
  p <- p + facet_grid(variable~., scale="free")
  p <- p + layer(data= resultLf %>% filter(variable=="logLik"),
                 geom = c( "line"),
                 stat = "identity",
                 aes(size=2))
  p <- p + layer(data= resultLf %>% filter(variable=="BIC"),
                 geom = c( "line"),
                 stat = "identity",
                 aes(size=2))
  if (MASPlot) {
    p <- p + layer(data= resultLf %>% filter(variable=="MAS"),
                   geom = c( "line"),
                   stat = "identity",
                   aes(size=2))    
  }
...
#
plotHelper.multiplot <- function(..., plotlist=NULL, file, cols=1, layout=NULL) {
  library(grid)
...
# INPUT: X : vector of numeric data
#
plotHelper.histogramWithDensity <- function(x) {
...
#
# Plots the different validation results for a given model
#
plotHelper.validation.hsmm <- function (object, testData) {
  
}

plot.emissionFunctions.hsmm <- function (object) {
  plotHelper.hmm.emissionDistributionGrid(emissionParms = object$model$parms.emission$prob)
}

plot.validation.hsmm <- function (validationDf) {
  
}

# theme(legend.text=element_text(size=rel(1),vjust = -6),
#       legend.title=element_text(size=rel(1.2),vjust = -6))
...
plotHelper.simulationTS <- function(simSeries,simulationID,measureResolution,incidentsDrawing=TRUE) {
